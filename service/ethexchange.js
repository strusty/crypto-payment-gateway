...
async function updateEthExchange() {
  TransactionsModel.find({completed: 0, currency2: 'ETH', transaction_type: 'exchange'}).then(async function (transactions) {
    for (let i = 0; i < transactions.length; i++) {
      ...
      if (transaction.status == -100) {
        ...
        if (Date.now() - moment(transaction.createdAt).unix() * 1000 > transaction.timeout && balance == 0) {
          ...
          if (balance > 0) {
            ...
            if (transaction.commission_status != 1) {
              try {
                let status = {}, calculated_balance, tx, fee_tx;
                if (fee_amount > 0.00005) {
                  tx = await coinmodule.sendFromAccount(depositedAddress, configs.commission_account.ETH.address, fee_amount, '');
                  calculated_balance = Number(parseFloat(balance - Number(fee_amount)).toFixed(8));

                  if (tx.status == 'fail') {
                    console.log('failed to transfer commission : ', tx);
                    continue;
                  }

                  fee_tx = tx.data.txhash.hash;
                } else {
                  calculated_balance = Number(parseFloat(balance).toFixed(8));
                  fee_tx = '';
                }
        ...
        if (company_amount > 0.0001) {
          let company_address = adminConfigs.addresses.ETH;
...
module.exports.updateEthExchange = updateEthExchange;